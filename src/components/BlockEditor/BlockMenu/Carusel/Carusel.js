...
const Carusel = ({ body, id, setViewEdit, vidjArray, setVidjetDataArray }) => {
        ...
        const CarouselList = ({ files, urls }) => {
            ...
            if (urls != null) {
                ...
                return urls.images.map((el, i) => {
                    return <CaruselItem delHandler = { delItem }
                    key = { i }
                    urlFile = { el }
                    index = { i }
                    />
        ...
        return ( <
            ...
            <
            CarouselList files = { files }
            urls = { body ? content : null }
            /> <
            /div> <
            input ref = { root }
            className = 'items-input-hidden'
            type = 'file'
            onChange = {
                (evt) => addFile(evt.target.files[0]) }
            /> <
            Button classes = {
                ['w-100 mw-100 text-left new-slide-button'] }
            disabled = {!isValidLimitSlide }
            onClick = { clickHandler }
            title = '+ новый слайд ' / > {!isValidLimitSlide ? < p className = 'text-danger' > Не больше трех слайдов < /p> : null} <
                ...
                <
                input type = 'number'
                className = ' question-item-input w-8 carusel-input-duration'
                value = { interval }
                onChange = {
                    (evt) => setInterval(evt.target.value) }
                /> <
                p className = 'items-label m-0' > Cек < /p> <
                /div> <
                /div> <
                /div> <
                /div> { /*   <div className='block-question-save'><p onClick={() => saveList()} className='block-question-button-save'>Сохранить</p></div> */ } <
                /PopUp>
            )
        }
